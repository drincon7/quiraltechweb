...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Add width and height variables to fix the errors
    let width = 0;
    let height = 0;

    // Pointer y configuración
    ...
    function resizeCanvas() {
      ...
      width = scaleByPixelRatio(section.clientWidth);
      height = scaleByPixelRatio(section.clientHeight);
    ...
    // Function to get relative position within the section
    function getRelativeMousePosition(e: MouseEvent | Touch) {
      const section = canvas?.closest('section');
      if (!section) {
    ...
    // Limitar eventos solo a la sección que lo contiene
    function isEventInContainer(e: MouseEvent | Touch): boolean {
      const section = canvas?.closest('section');
      if (!section) return false;
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        if (!isEventInContainer(touches[i])) continue;
        
        const pos = getRelativeMousePosition(touches[i]);
        const posX = scaleByPixelRatio(pos.x);
        const posY = scaleByPixelRatio(pos.y);
    ...
    // Add event listeners to the document (will filter based on container)
